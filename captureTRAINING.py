...
class CaptureRules:
  ...
  def process(self, state, game):
    ...
    # Set this for training purposes. Only want to train getting the first 8 food.
    # if len(game.moveHistory) > 8:
    #   if game.agents[0].getNumFoodEaten() >= 8:
    #     state.data._win = True
...
def runGames(layouts, agents, display, length, numGames, record, numTraining, redTeamName, blueTeamName,
  ...
  for i in range( numGames ):
    ...
    if beQuiet:
        ...
        gameDisplay = display
        rules.quiet = False
    # HACK for me to visualise for debugging
    from captureGraphicsDisplay import PacmanGraphics
    gameDisplay = PacmanGraphics([agents[0], agents[2]], redTeamName, [agents[1], agents[3]], blueTeamName)
